...
router.get('/orders', (req, res) => {
  const { status, limit = 50, api_key_id, dashboard_id, search, from, to, format, offset = 0 } = req.query;
  let query = `
  ...
  if (dashboard_id) { query += ` AND k.dashboard_id = ?`; params.push(dashboard_id); }
  if (search) { query += ` AND (o.id LIKE ? OR k.label LIKE ?)`; params.push(`${search}%`, `%${search}%`); }
  ...
  if (format === 'csv') {
    ...
    const cols = ['id', 'created_at', 'status', 'amount_usdc', 'payment_asset', 'api_key_label', 'card_brand', 'stellar_txid', 'error'];
...
router.get('/stats', (req, res) => {
  ...
  const activeKeys = /** @type {any} */ (db.prepare(`SELECT COUNT(*) AS count FROM api_keys WHERE enabled = 1`).get());
...
router.get('/health', (req, res) => {
  const systemState = Object.fromEntries(
    /** @type {any[]} */ (db.prepare(`SELECT key, value FROM system_state`).all()).map(r => [r.key, r.value])
  );

  // Order pipeline status distribution — the shape of the operational pipeline
  const orderStates = /** @type {any[]} */ (db.prepare(`
  ...
  const webhookHealth = db.prepare(`
    SELECT
  ...
  const recentAdminActions = /** @type {any[]} */ (db.prepare(`
  ...
  const retentionState = db.prepare(`
    SELECT
...
router.post('/system/unfreeze', requireOwner, (req, res) => {
  ...
  db.prepare(`INSERT OR REPLACE INTO system_state (key, value) VALUES ('consecutive_failures', '0')`).run();
...
function validateApiKeyFields({ spend_limit_usdc, default_webhook_url, wallet_public_key,
  policy_daily_limit_usdc, policy_single_tx_limit_usdc, policy_require_approval_above_usdc,
  policy_allowed_hours, policy_allowed_days, mode, rate_limit_rpm, expires_at }) {
  if (spend_limit_usdc !== undefined && spend_limit_usdc !== null) {
    if (!/^\d+(\.\d+)?$/.test(String(spend_limit_usdc)) || parseFloat(spend_limit_usdc) <= 0) {
      return { error: 'invalid_spend_limit', message: 'spend_limit_usdc must be a positive decimal (e.g. "100.00")' };
    }
  }
  if (default_webhook_url !== undefined && default_webhook_url !== null && default_webhook_url !== '') {
    try {
      const u = new URL(default_webhook_url);
      if (u.protocol !== 'https:') return { error: 'invalid_webhook_url', message: 'default_webhook_url must use HTTPS' };
    } catch {
  ...
  if (wallet_public_key !== undefined && wallet_public_key !== null && wallet_public_key !== '') {
    if (!/^G[A-Z2-7]{55}$/.test(wallet_public_key)) {
      return { error: 'invalid_wallet_public_key', message: 'wallet_public_key must be a valid Stellar G-address (56 chars, starts with G)' };
    }
  }
  ...
  if (policy_allowed_hours !== undefined && policy_allowed_hours !== null && policy_allowed_hours !== '') {
    try {
      const h = JSON.parse(policy_allowed_hours);
      if (!/^\d{2}:\d{2}$/.test(h.start) || !/^\d{2}:\d{2}$/.test(h.end)) throw new Error();
    } catch {
      return { error: 'invalid_policy', message: 'policy_allowed_hours must be JSON like {"start":"09:00","end":"17:00"}' };
    }
  }
  if (policy_allowed_days !== undefined && policy_allowed_days !== null && policy_allowed_days !== '') {
    try {
      const d = JSON.parse(policy_allowed_days);
      if (!Array.isArray(d) || d.some(n => n < 0 || n > 6)) throw new Error();
    } catch {
      return { error: 'invalid_policy', message: 'policy_allowed_days must be JSON array of day numbers 0–6 (e.g. [1,2,3,4,5])' };
    }
  }
  ...
  if (rate_limit_rpm !== undefined && rate_limit_rpm !== null && rate_limit_rpm !== '') {
    ...
    if (isNaN(rpm) || rpm < 1 || rpm > 10000) {
      return { error: 'invalid_rate_limit', message: 'rate_limit_rpm must be an integer between 1 and 10000' };
    }
  }
...
router.post('/api-keys', requireOwner, async (req, res) => {
  ...
  const validationErr = validateApiKeyFields(/** @type {any} */ ({ spend_limit_usdc, default_webhook_url, wallet_public_key }));
  if (validationErr) return res.status(400).json(validationErr);
  if (default_webhook_url) {
    try { await assertSafeUrl(default_webhook_url); }
    catch (err) { return res.status(400).json({ error: 'invalid_webhook_url', message: err.message }); }
  }
  const id = uuidv4();
  ...
  `).run({ id, keyHash, keyPrefix, label: label || null, spend_limit_usdc: spend_limit_usdc || null, webhook_secret: webhookSecret, default_webhook_url: default_webhook_url || null, wallet_public_key: wallet_public_key || null });
...
router.patch('/api-keys/:id', async (req, res) => {
  const {
    enabled, spend_limit_usdc, default_webhook_url, label, wallet_public_key,
    policy_daily_limit_usdc, policy_single_tx_limit_usdc, policy_require_approval_above_usdc,
    policy_allowed_hours, policy_allowed_days, mode, rate_limit_rpm, expires_at,
  } = req.body;
  const validationErr = validateApiKeyFields({
    spend_limit_usdc, default_webhook_url, wallet_public_key,
    policy_daily_limit_usdc, policy_single_tx_limit_usdc, policy_require_approval_above_usdc,
    policy_allowed_hours, policy_allowed_days, mode, rate_limit_rpm, expires_at,
  });
  if (validationErr) return res.status(400).json(validationErr);
  ...
  if (policy_daily_limit_usdc !== undefined) fields.policy_daily_limit_usdc = policy_daily_limit_usdc || null;
  if (policy_single_tx_limit_usdc !== undefined) fields.policy_single_tx_limit_usdc = policy_single_tx_limit_usdc || null;
  if (policy_require_approval_above_usdc !== undefined) fields.policy_require_approval_above_usdc = policy_require_approval_above_usdc || null;
  if (policy_allowed_hours !== undefined) fields.policy_allowed_hours = policy_allowed_hours || null;
  ...
  if (rate_limit_rpm !== undefined) fields.rate_limit_rpm = rate_limit_rpm ? parseInt(rate_limit_rpm) : null;
  ...
  const result = db.prepare(`UPDATE api_keys SET ${sets} WHERE id = @id`).run({ id: req.params.id, ...fields });
...
router.post('/api-keys/:id/rotate', requireOwner, async (req, res) => {
  ...
  db.prepare(`UPDATE api_keys SET key_hash = ?, key_prefix = ? WHERE id = ?`).run(keyHash, keyPrefix, req.params.id);
...
router.post('/api-keys/:id/test-webhook', async (req, res) => {
  const key = /** @type {any} */ (db.prepare(`SELECT * FROM api_keys WHERE id = ?`).get(req.params.id));
  if (!key) return res.status(404).json({ error: 'not_found' });
  if (!key.default_webhook_url) return res.status(422).json({ error: 'no_webhook_configured', message: 'Set a default_webhook_url on this key first.' });

  try {
    await fireWebhook(key.default_webhook_url, {
      event: 'test',
      order_id: `test-${Date.now()}`,
      status: 'test',
      message: 'Test webhook from cards402 admin.',
    }, key.webhook_secret || null, null);
...
router.post('/api-keys/:id/suspend', requireOwner, (req, res) => {
  ...
  const pendingApprovals = /** @type {any[]} */ (db.prepare(`SELECT * FROM approval_requests WHERE api_key_id = ? AND status = 'pending'`).all(req.params.id));
  for (const approval of pendingApprovals) {
    db.prepare(`UPDATE approval_requests SET status = 'rejected', decided_at = ?, decision_note = ? WHERE id = ?`)
      .run(now, 'Agent suspended by owner', approval.id);
    db.prepare(`UPDATE orders SET status = 'rejected', error = ?, updated_at = ? WHERE id = ?`)
      .run('Agent suspended by owner', now, approval.order_id);
    recordDecision(req.params.id, approval.order_id, approval.amount_usdc, 'blocked', 'suspended', 'Agent suspended by owner');
  }
  bizEvent('admin.agent_suspended', { api_key_id: req.params.id, actor: req.user.email, cancelled_approvals: pendingApprovals.length });
...
router.post('/approval-requests/:id/approve', async (req, res) => {
  const approval = /** @type {any} */ (db.prepare(`SELECT * FROM approval_requests WHERE id = ?`).get(req.params.id));
  if (!approval) return res.status(404).json({ error: 'not_found' });
  if (approval.status !== 'pending') return res.status(409).json({ error: 'already_decided', current_status: approval.status });
  // B-4: reject expired approvals — avoids dispatching a card after the approval window closed
  if (new Date(approval.expires_at) <= new Date()) return res.status(410).json({ error: 'approval_expired' });
  ...
  const now = new Date().toISOString();
  db.prepare(`UPDATE approval_requests SET status = 'approved', decided_at = ?, decided_by = ?, decision_note = ? WHERE id = ?`)
  ...
  recordDecision(approval.api_key_id, approval.order_id, approval.amount_usdc, 'approved', 'owner_approved',
    req.body.note || 'Approved by account owner');

  // Notify agent via webhook if configured
  const order = /** @type {any} */ (db.prepare(`SELECT * FROM orders WHERE id = ?`).get(approval.order_id));
  const keyRow = /** @type {any} */ (db.prepare(`SELECT webhook_secret, default_webhook_url FROM api_keys WHERE id = ?`).get(approval.api_key_id));
  const webhookUrl = order?.webhook_url || keyRow?.default_webhook_url || null;
  if (webhookUrl) {
    enqueueWebhook(webhookUrl, { order_id: approval.order_id, status: 'pending_payment', phase: 'awaiting_payment', note: 'Transaction approved by owner.' }, keyRow?.webhook_secret || null)
      .catch(() => {});
  }

  bizEvent('admin.approval_approved', { approval_id: req.params.id, order_id: approval.order_id, actor: req.user.email });
...
router.post('/approval-requests/:id/reject', (req, res) => {
  ...
  db.prepare(`UPDATE approval_requests SET status = 'rejected', decided_at = ?, decided_by = ?, decision_note = ? WHERE id = ?`)
    .run(now, req.user.email, note, req.params.id);
  db.prepare(`UPDATE orders SET status = 'rejected', error = ?, updated_at = ? WHERE id = ?`)
    .run(note, now, approval.order_id);

  recordDecision(approval.api_key_id, approval.order_id, approval.amount_usdc, 'blocked', 'owner_rejected', note);
  ...
  const webhookUrl = order?.webhook_url || keyRow?.default_webhook_url || null;
  if (webhookUrl) {
    enqueueWebhook(webhookUrl, { order_id: approval.order_id, status: 'rejected', phase: 'rejected', error: note }, keyRow?.webhook_secret || null)
      .catch(() => {});
  }

  bizEvent('admin.approval_rejected', { approval_id: req.params.id, order_id: approval.order_id, actor: req.user.email });
...
router.get('/admin-actions', (req, res) => {
  const { actor_email, action, target_type, target_id, from, to, limit = 200, offset = 0 } = req.query;
  let query = `
    SELECT id, actor_email, action, target_type, target_id, metadata, ip, request_id, created_at
    FROM admin_actions WHERE 1=1
  `;
  const params = [];
  if (actor_email) { query += ` AND actor_email = ?`; params.push(actor_email); }
...
router.get('/policy-decisions', (req, res) => {
  ...
  `;
  const params = [];
  if (api_key_id) { query += ` AND pd.api_key_id = ?`; params.push(api_key_id); }
  if (decision) { query += ` AND pd.decision = ?`; params.push(decision); }
  ...
  if (format === 'csv') {
    const cols = ['id', 'created_at', 'api_key_label', 'api_key_id', 'decision', 'rule', 'amount_usdc', 'reason', 'order_id'];
    const escape = (v) => (v === null || v === undefined) ? '' : `"${String(v).replace(/"/g, '""')}"`;
    const lines = [
      cols.join(','),
...
router.post('/webhooks/:id/retry', async (req, res) => {
  const row = /** @type {any} */ (db.prepare(`SELECT * FROM webhook_queue WHERE id = ?`).get(req.params.id));
  ...
  try {
    ...
    const now = new Date().toISOString();
    db.prepare(`UPDATE webhook_queue SET attempts = attempts + 1, last_error = ?, next_attempt = ? WHERE id = ?`)
...
router.post('/orders/:id/refund', requireOwner, async (req, res) => {
  const order = /** @type {any} */ (db.prepare(`SELECT * FROM orders WHERE id = ?`).get(req.params.id));
  if (!order) return res.status(404).json({ error: 'not_found' });
  bizEvent('admin.manual_refund', { order_id: order.id, status: order.status, actor: req.user.email });
...
router.delete('/users/:id', requireOwner, (req, res) => {
  const target = /** @type {any} */ (db.prepare(`SELECT * FROM users WHERE id = ?`).get(req.params.id));
  if (!target) return res.status(404).json({ error: 'not_found' });
  if (target.role === 'owner') return res.status(403).json({ error: 'cannot_remove_owner', message: 'Transfer ownership first.' });
