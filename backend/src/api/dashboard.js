...
router.get('/stats', (req, res) => {
  ...
  const activeKeys = db.prepare(
    `SELECT COUNT(*) AS n FROM api_keys WHERE dashboard_id = ? AND enabled = 1`
...
router.get('/orders', (req, res) => {
  ...
  if (api_key_id) {
    ...
    const owns = db.prepare(`SELECT 1 FROM api_keys WHERE id = ? AND dashboard_id = ?`).get(String(api_key_id), req.dashboard.id);
    if (!owns) return res.status(403).json({ error: 'forbidden' });
    query += ` AND o.api_key_id = ?`; params.push(String(api_key_id));
  }
...
router.post('/api-keys', async (req, res) => {
  ...
  bizEvent('dashboard.key_created', { dashboard_id: req.dashboard.id, api_key_id: id, actor: req.user.email });
...
router.patch('/api-keys/:id', async (req, res) => {
  const owned = db.prepare(`SELECT id FROM api_keys WHERE id = ? AND dashboard_id = ?`).get(req.params.id, req.dashboard.id);
  ...
  db.prepare(`UPDATE api_keys SET ${sets} WHERE id = @id AND dashboard_id = @dashboard_id`)
    .run({ id: req.params.id, dashboard_id: req.dashboard.id, ...fields });
...
router.delete('/api-keys/:id', (req, res) => {
  const result = db.prepare(`DELETE FROM api_keys WHERE id = ? AND dashboard_id = ?`).run(req.params.id, req.dashboard.id);
  if (result.changes === 0) return res.status(404).json({ error: 'not_found' });
  bizEvent('dashboard.key_deleted', { dashboard_id: req.dashboard.id, api_key_id: req.params.id, actor: req.user.email });
  res.json({ ok: true });
});

// POST /dashboard/api-keys/:id/suspend
router.post('/api-keys/:id/suspend', (req, res) => {
  const result = db.prepare(`UPDATE api_keys SET suspended = 1 WHERE id = ? AND dashboard_id = ?`).run(req.params.id, req.dashboard.id);
  if (result.changes === 0) return res.status(404).json({ error: 'not_found' });

  const now = new Date().toISOString();
  const pendingApprovals = db.prepare(`SELECT * FROM approval_requests WHERE api_key_id = ? AND status = 'pending'`).all(req.params.id);
  for (const approval of pendingApprovals) {
    ...
    recordDecision(req.params.id, approval.order_id, approval.amount_usdc, 'blocked', 'suspended', 'Agent suspended');
  }
  bizEvent('dashboard.agent_suspended', { dashboard_id: req.dashboard.id, api_key_id: req.params.id, actor: req.user.email });
  res.json({ ok: true });
});

// POST /dashboard/api-keys/:id/unsuspend
router.post('/api-keys/:id/unsuspend', (req, res) => {
  const result = db.prepare(`UPDATE api_keys SET suspended = 0 WHERE id = ? AND dashboard_id = ?`).run(req.params.id, req.dashboard.id);
  if (result.changes === 0) return res.status(404).json({ error: 'not_found' });
  bizEvent('dashboard.agent_unsuspended', { dashboard_id: req.dashboard.id, api_key_id: req.params.id, actor: req.user.email });
  res.json({ ok: true });
});
...
router.post('/approval-requests/:id/approve', async (req, res) => {
  ...
  const keyRow = db.prepare(`SELECT webhook_secret, default_webhook_url FROM api_keys WHERE id = ?`).get(approval.api_key_id);
  const webhookUrl = order?.webhook_url || keyRow?.default_webhook_url || null;
  if (webhookUrl) {
    enqueueWebhook(webhookUrl,
      { order_id: approval.order_id, status: 'pending_payment', phase: 'awaiting_payment', note: 'Approved.' },
...
router.get('/policy-decisions', (req, res) => {
  ...
  `;
  const params = [req.dashboard.id];
  if (api_key_id) { query += ` AND pd.api_key_id = ?`; params.push(String(api_key_id)); }
  if (decision) { query += ` AND pd.decision = ?`; params.push(String(decision)); }
