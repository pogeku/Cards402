...
- **[CRITICAL] A-1.** Backend is plain JavaScript, not TypeScript —
  ...
  *Status: partially done 2026-04-12 — OpenAPI specs shipped (C-17) in
  ...
  `contract/api/*.d.ts`. CI step validates spec + detects generated type
...
- **[CRITICAL] A-2.** SMTP vars not validated in env schema — `backend/src/env.js`
  ...
  *Status: done 2026-04-12 — added SMTP_HOST/PORT/USER/PASS/FROM to the
  ...
  config fails startup with a clear message naming the missing vars.*
...
- **[CRITICAL] A-4.** VCC token deletion without error context —
  ...
  *Status: done 2026-04-12 — all three 401 paths (`getInvoice`,
  ...
  response before deleting the token.*
...
- **[CRITICAL] A-5.** No schema-version check at startup — `backend/src/env.js`.
  ...
  *Status: done 2026-04-12 — `db.js` now declares
  ...
  documents the bump-in-lockstep rule.*
...
- **[HIGH] A-6.** Admin session key generation undocumented —
  ...
  *Status: done 2026-04-12 — added `scripts/generate-admin-key.js` (ESM
  module using `crypto.randomBytes(32).toString('hex')`). CONTRIBUTING.md
  references it. env.js now validates the shape (64-hex) when provided.*
...
- **[HIGH] A-7.** No per-URL webhook rate limit / circuit breaker —
  ...
  *Status: done 2026-04-12 — added a per-origin circuit breaker in
  ...
  breaker alone blocks one slow customer from stalling every other.*
...
- **[HIGH] A-8.** No logging of idempotency cache hits —
  ...
  *Status: done 2026-04-12 — `idempotency.cache_hit` and
...
- **[HIGH] A-9.** CSV export has no pagination / streaming —
  ...
  *Status: done 2026-04-12 — CSV path now uses better-sqlite3's
  `iterate()` to stream rows directly to `res.write()` without
  materialising the result set. JSON path got a proper `LIMIT ... OFFSET`
  with a hard cap of 500; CSV allows up to 20k for export use cases.*
...
- **[HIGH] A-10.** Policy after-hours check silently skips on malformed JSON —
  ...
  **Fix:** Validate JSON at storage time; throw at eval time if present and
  invalid.
  *Status: done 2026-04-12 — policy.js now FAILS CLOSED on malformed
  ...
  dashboard.js and admin.js already existed; this is defense in depth.*
...
- **[HIGH] A-11.** Webhook verification timing-safe compare audit needed —
  ...
  *Status: done 2026-04-12 — extracted `backend/src/lib/hmac.js` (and
  ...
  inputs. Both vcc (sign) and cards402 (verify) now use the shared lib.*
...
- **[HIGH] A-12.** `stellarStartLedger` persists without fsync —
  ...
  *Status: done 2026-04-12 — `saveStartLedger` now runs
  ...
  braces. Wrapped in try/catch so a concurrent checkpoint doesn't bubble.*
...
- **[HIGH] A-13.** `/admin/orders` has no `dashboard_id` filter —
  ...
  *Status: done 2026-04-12 — added optional `?dashboard_id=` query filter
  (JOINs via `api_keys.dashboard_id`). Also added `?offset=` pagination
  so the UI doesn't rely on LIMIT-only scrolling.*
...
- **[MEDIUM] A-15.** Backend `dist/` gitignore audit — confirm compiled JS is
  ...
  *Status: done 2026-04-12 — verified: `.gitignore` has
...
- **[MEDIUM] A-16.** SDK has no integration test against real backend —
  ...
  *Status: done 2026-04-12 — added `sdk/src/__tests__/integration.test.ts`
  ...
  logic (503 → success on second attempt). SDK total: 54 → 60 tests.*
...
- **[MEDIUM] A-17.** Admin dashboard has no audit log —
  ...
  *Status: done 2026-04-12 — new `admin_actions` table (migration 12) +
  ...
  timestamp. 3 new tests cover the insert path + filtering.*
...
- **[MEDIUM] A-18.** Duplicate stats queries in dashboard and admin —
  ...
  *Status: done 2026-04-12 — extracted `lib/stats.js::getOrderStats(opts)`
  ...
  queries next change — the helper is there and tested.*
...
- **[MEDIUM] A-19.** `GET /v1/orders` has no date filter —
  ...
  *Status: done 2026-04-12 — `GET /v1/orders` now accepts
  ...
  `Cards402Client.listOrders()` surfaces both new filters.*
...
- **[MEDIUM] A-20.** Admin UI missing key operational metrics —
  ...
  *Status: done 2026-04-12 — new `GET /admin/health` endpoint returns
  ...
  this is tracked separately — the backend endpoint is the blocker.)*
...
- **[MEDIUM] A-21.** Recovery-job env vars undocumented —
  ...
  *Status: done 2026-04-12 — added to Zod as optional numeric strings so
...
- **[MEDIUM] A-22.** SDK MCP server has no input validation —
  ...
  *Status: done 2026-04-12 — verified: `purchase_vcc`, `check_order` all
...
- **[MEDIUM] A-23.** `Cards402Client` has no retry on network errors —
  ...
  *Status: done 2026-04-12 — added `fetchWithRetry` helper with
  ...
  accepts a `retry` option for tuning per-agent.*
...
- **[MEDIUM] A-24.** Policy approval timeout is hardcoded — `backend/src/jobs.js`
  ...
  **Fix:** `approval_ttl_minutes` on dashboard or api_key.
  *Status: done 2026-04-12 — added `APPROVAL_TTL_MINUTES` env var
...
- **[MEDIUM] A-25.** No HTTPS enforcement middleware — `backend/src/app.js`.
  ...
  *Status: done 2026-04-12 — `app.js` now has an HTTPS-only middleware
...
- **[LOW] A-26.** Inconsistent casing in order fields —
  ...
  **Fix:** Stick to snake_case on the wire; JSDoc types.
  *Status: deferred — convention is snake_case on the HTTP wire,
...
- **[LOW] A-27.** SDK exports both `payViaContract` and `payVCC` —
  ...
  *Status: done 2026-04-12 — verified both aliases already carry JSDoc
  ...
  deferred to a v1.0 major release per semver.*
...
- **[LOW] A-28.** Web admin has no loading state on long ops —
  ...
  *Status: done 2026-04-12 — added `useAction` hook to admin page.
...
- **[LOW] A-30.** No dead-letter queue for failed webhooks —
  ...
  *Status: partially addressed 2026-04-12 — the webhook circuit
  ...
  equivalent and shipped this sweep.*
...
- **[LOW] A-31.** No delegation of approval authority —
  ...
  *Status: deferred — product decision. Audit sweep focused on
  technical hardening; delegation is a UX/permissions scope for a
  later product iteration.*
...
- **[LOW] A-32.** No pagination on order-list endpoints —
  ...
  *Status: done 2026-04-12 — addressed alongside A-19 and A-13. Agent-
  facing `GET /v1/orders` and admin `GET /admin/orders` both accept
  `offset` + `limit`, with `since_*` filters on the agent side. Cursor-
...
- **[LOW] A-33.** Webhook payload doesn't include amount —
  ...
  *Status: done 2026-04-12 — both the fulfilled and failed webhook
...
- **[LOW] A-34.** SDK test coverage unclear in CI — `sdk/__tests__/`.
  ...
  *Status: done 2026-04-12 — verified: `.github/workflows/ci.yml`
...
- **[NIT] A-40.** Admin dashboard pagination state not persisted in URL —
  ...
  *Status: done 2026-04-12 — admin page now syncs `orderSearch`,
  `orderFrom`, `orderTo`, `filterKey` to URL search params via
  `useSearchParams` + `router.replace`. Browser refresh and
  back-button preserve the operator's current filter state.*
...
- **[CRITICAL] B-1.** CTX access + refresh tokens plaintext in `system_state` —
  ...
  *Status: done 2026-04-12 — `loadTokens`/`saveTokens` now wrap
  ...
  encrypted. `VCC_DATA_KEY` is required in production per env.js.*
...
- **[CRITICAL] B-2.** POST `/api/register` has no rate limit —
  ...
  *Status: done 2026-04-12 — 10 registrations per IP per hour via a
  dedicated limiter mounted before the register router. Returns 429 with
  a `rate_limited` error body.*
...
- **[HIGH] B-4.** Admin routes lack per-token rate limiting —
  ...
  *Status: done 2026-04-12 — dedicated `adminLimiter` at 100 req/min,
  ...
  the full secret — is stored as the rate-limit key.*
...
- **[HIGH] B-5.** Unused `raceSolversV3`/`raceSolversHCaptcha` exports —
  ...
  *Status: done 2026-04-12 — deleted both racer functions and their exports,
  simplified module header comment.*
...
- **[HIGH] B-6.** ~200 LOC of dead image-classification code —
  ...
  **Fix:** Delete.
  *Status: done 2026-04-12 — rewrote `challenge.js` from ~300 LOC to ~80 LOC,
  ...
  file headers updated to reflect v2-only support.*
...
- **[HIGH] B-7.** Callback handshake is one-directional —
  ...
  **Fix:** At minimum document it; ideally mTLS or cert pinning for callback
  channel.
  *Status: documented as design limitation 2026-04-12 — see
  ...
  binding and 10-min replay window is the mitigation until then.*
...
- **[HIGH] B-8.** CTX token refresh has no infinite-loop guard —
  ...
  *Status: done 2026-04-12 — module-level `_lastRefreshAttempt` +
  ...
  pointing at `scripts/ctx-auth.js`. Successful refresh resets the error.*
...
- **[MEDIUM] B-10.** Callback retry budget too short —
  ...
  *Status: done 2026-04-12 — fully addressed by the callback_deadletter
  ...
  latency for marginal gains now that the DLQ exists.*
...
- **[MEDIUM] B-11.** Retention sweep silent failure on decrypt —
  ...
  *Status: done 2026-04-12 — `retention.decrypt_failed` bizEvent emitted
...
- **[MEDIUM] B-12.** `YOURREWARDCARD_API_KEY` optional but may be required —
  ...
  *Status: done 2026-04-12 — improved the runtime error to tell operators
  ...
  the extraction regresses, the runtime error is now self-explanatory.*
...
- **[MEDIUM] B-13.** Stage1-direct fallback swallows errors —
  ...
  *Status: done 2026-04-12 — emits three bizEvents:
  ...
  now watch the fallback rate to decide whether to roll back the flag.*
...
- **[MEDIUM] B-14.** `FREEZE_THRESHOLD=3` hardcoded, no alerting —
  ...
  *Status: done 2026-04-12 — `VCC_FREEZE_THRESHOLD` env var now tunes
  the threshold (default 3). Freeze trigger emits `fulfillment.frozen`
  bizEvent and a Discord embed via `notifyOps({ type: 'frozen' })`.*
...
- **[MEDIUM] B-15.** Stage1 timeout is per-attempt, not cumulative —
  ...
  *Status: done 2026-04-12 — added `STAGE1_TOTAL_TIMEOUT_MS` (default
  180000) checked at the top of each attempt loop iteration. Bails
  ...
  stage1 gives up. Still leaves headroom under the 10-min watchdog.*
...
- **[MEDIUM] B-16.** Zero scraper test coverage — `vcc/api/test/`. No tests for
  ...
  *Status: done 2026-04-12 — added three unit test suites:
...
- **[NIT] B-17.** Unused CapSolver builder call in unreachable code —
  ...
  **Fix:** Delete with B-6.
  *Status: done 2026-04-12 — deleted together with B-6.*
...
- **[NIT] B-18.** Confusing token pre-solve comment —
  ...
  *Status: done 2026-04-12 — comment now explicitly calls out that the
  browser path is audio-only and the direct-API path still uses v2
  token solving (via a fundamentally different request flow).*
...
- **[NIT] B-19.** `admin_audit` table has no retention policy —
  ...
  *Status: done 2026-04-12 — SQL comment block in the table definition
  explicitly documents "no automatic rotation — this is a compliance
  log" with an example DELETE cron for operators who need to trim it.*
...
- **[NIT] B-20.** Discord webhook errors swallowed —
  ...
  *Status: done 2026-04-12 — added a comment block above `notifyOps`
...
- **[NIT] B-21.** Bearer token length not validated —
  ...
  *Status: done 2026-04-12 — `requireAuth` now rejects non-string,
  too-short (<20), or too-long (>256) tokens before the DB lookup.*
...
- **[NIT] B-24.** Graceful shutdown doesn't wait for in-flight jobs —
  ...
  *Status: done 2026-04-12 — `fulfillment.js` exports
...
- **[NIT] B-25.** No `/metrics` endpoint — `vcc/api/src/app.js`. Only `/health`.
  ...
  *Status: done 2026-04-12 — added `GET /metrics` that emits JSON with
  ...
  an external scraper ever wants it.*

- **[NIT] B-26.** No README.md in `vcc/api/`
  **Fix:** Write one.
  *Status: done 2026-04-12 — added `vcc/api/README.md` with bootstrap +
  operator runbook, env var reference, and a docs map pointing at
  cards402's ARCHITECTURE.md as the canonical cross-repo doc.*

- **[NIT] B-27.** No ARCHITECTURE.md in vcc
  **Fix:** Write one.
  *Status: done 2026-04-12 — added `vcc/api/docs/ARCHITECTURE.md` with
...
- **[NIT] B-30.** `.env.example` missing `CTX_STREAM`, `STAGE1_DIRECT_API`,
  ...
  *Status: done 2026-04-12 — added CTX_STREAM, STAGE1_DIRECT_API,
...
- **[CRITICAL] C-1.** No correlation / trace ID across the boundary —
  ...
  manually correlate by order_id across two services.
  **Fix:** Generate `req_<ulid>` in cards402; send as `X-Request-ID`; propagate
  through vcc + scraper logs.
  *Status: done 2026-04-12 — end-to-end. cards402 stores `req.id` on
  `orders.request_id` at creation (migration 11). Dispatch path passes it
  ...
  reads the job's request id so every scraper line carries
  `[job:abcd1234 req:req_abcd]`. vcc echoes the same id in the outbound
  callback as `X-Request-ID`; cards402's `/vcc-callback` handler logs a
  `callback.received` bizEvent pairing the upstream id with the local one
...
- **[CRITICAL] C-2.** No explicit retry budget / dead-letter for callbacks —
  ...
  *Status: done 2026-04-12 — `callback_deadletter` table added via
  ...
  failed retry appends the error and bumps `attempts`.*
...
- **[CRITICAL] C-3.** Callback secret is tenant-wide and long-lived —
  ...
  **Fix:** Per-job nonce at invoice time; sign nonce+order_id into scoped
  token; rotate master on 90d cadence.
  *Status: done 2026-04-12 — full per-job nonce scoping shipped as v3
...
- **[CRITICAL] C-4.** Callback signature doesn't cover `order_id` —
  ...
  **Fix:** Include `order_id` in signing material; verify request path matches
  body on both sides.
  *Status: done 2026-04-12 — v2 protocol binds `order_id` into the HMAC
...
- **[CRITICAL] C-5.** Clock skew tolerance hardcoded 5min, asymmetric —
  ...
  **Fix:** 10min window; NTP sync guidance in deploy docs; metric on stale
  rejections.
  *Status: done 2026-04-12 — window increased to 10 minutes in `lib/hmac.js`
  ...
  documented in deploy runbook (rolled into the docs sweep).*
...
- **[HIGH] C-6.** vcc callback not DB-idempotent —
  ...
  *Status: done 2026-04-12 — rewrote both the `status === 'fulfilled'`
  ...
  of double-writing card data. No schema change needed.*
...
- **[HIGH] C-7.** No circuit breaker / fail-fast when vcc is down —
  ...
  *Status: done 2026-04-12 — added in-memory circuit breaker on
  ...
  trace the `VCC invoice failed` error spam.*
...
- **[HIGH] C-8.** Callback secret plaintext in dev/staging —
  ...
  *Status: done 2026-04-12 — production already hard-required. Dev/stage
...
- **[HIGH] C-9.** No API versioning between cards402 and vcc —
  ...
  *Status: done 2026-04-12 — `GET /api/version` endpoint added to both
  ...
  in `verifyCallback` demonstrates the forward-compat rollout pattern.*
...
- **[HIGH] C-10.** Callback retry timestamps non-deterministic —
  ...
  *Status: done 2026-04-12 — added multi-line comment above the
  ...
  copy-paste a signature from logs; they must regenerate ts + HMAC).*
...
- **[MEDIUM] C-11.** No explicit body-size limit on vcc callback route —
  ...
  *Status: done 2026-04-12 — already fixed: cards402's global
...
- **[MEDIUM] C-12.** No separate rate-limit bucket for GET vs POST —
  ...
  *Status: done 2026-04-12 — replaced the single tenant limiter with
...
- **[MEDIUM] C-13.** `VCC_CALLBACK_SECRET` length not validated in cards402 —
  ...
  *Status: done 2026-04-12 — bumped Zod `min(16)` → `min(32)` with an
...
- **[MEDIUM] C-14.** vcc frozen state is global, not per-tenant —
  ...
  *Status: done 2026-04-12 — `frozenKey(tenantId)` /
  ...
  ops can still halt all fulfillment in a single write.*
...
- **[MEDIUM] C-15.** Callback URL not validated at invoice time —
  ...
  *Status: done 2026-04-12 — invoice handler now runs `assertSafeUrl`
  ...
  `ssrf.js` so `.test` domains don't need DNS resolution during tests.*
...
- **[NIT] C-16.** E2E fake vcc server is single-threaded —
  ...
  *Status: partially addressed 2026-04-12 — new cross-repo CI workflow
...
- **[NIT] C-17.** No OpenAPI / JSON schema contract — neither repo has
  ...
  **Fix:** `/Users/ash/code/cards402/contract/vcc-api.openapi.yaml` + codegen
  both sides.
  *Status: done 2026-04-12 — two OpenAPI 3.1 specs shipped:
  ...
  TypeScript types auto-generated via `openapi-typescript` into
  `contract/api/*.d.ts` with `npm run api:generate`. CI workflow
...
- **[NIT] C-18.** No documented SLO/SLA for callback delivery — neither
  ...
  *Status: done 2026-04-12 — `cards402/ARCHITECTURE.md` §"Callback
  ...
  the corresponding vcc side.*

- **[NIT] C-19.** No `callback_deadletter` table / webhook history.
  **Fix:** Combines with C-2.
  *Status: done 2026-04-12 — merged into C-2 closure. See
  `vcc/api/src/db.js` migration 7 and the new admin endpoints.*
...
- **[NIT] C-20.** No migration test for `VCC_DATA_KEY_PREVIOUS` rotation —
  ...
  *Status: done 2026-04-12 — `test/unit/crypto-rotation.test.js` covers
  ...
  null/undefined handling. 7 new tests.*
