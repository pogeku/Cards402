...
const orderStatuses = [
  ...
  {
    status: 'failed',
    meaning: 'Fulfillment failed. The error field contains the reason. A refund is automatically queued.',
  },
  {
    status: 'refunded',
    meaning: 'Payment refunded to your sender address. The refund_stellar_txid field has the transaction hash.',
  },
  {
    status: 'expired',
...
const errors = [
  ...
  { code: 'invalid_amount', status: 400, meaning: 'amount_usdc is missing or not a positive number.' },
  { code: 'invalid_payment_asset', status: 400, meaning: 'payment_asset must be "usdc" or "xlm".' },
  { code: 'spend_limit_exceeded', status: 403, meaning: 'This key has reached its USDC spend limit.' },
  { code: 'order_not_found', status: 404, meaning: 'Order does not exist or belongs to another key.' },
  { code: 'xlm_price_unavailable', status: 503, meaning: 'XLM price feed unavailable. Retry in a few seconds or use USDC instead.' },
  { code: 'service_temporarily_unavailable', status: 503, meaning: 'Circuit breaker tripped after repeated failures. Try again later.' },
];
    ...
    >
      ...
      >
        ...
        <Section id="authentication">
          ...
          <Para>
            If the key is missing, invalid, or disabled, the API returns <Code>401 Unauthorized</Code>.
            If the key has a spend limit and it is exceeded, the API returns{' '}
        ...
        <Section id="create-order">
          ...
          <Para>Content-Type: <Code>application/json</Code></Para>
          ...
          <div style={{ marginBottom: '1.5rem', overflowX: 'auto' }}>
            <table>
              ...
              <tbody>
                <tr>
                  <td><Code>amount_usdc</Code></td>
                  <td style={{ color: 'var(--muted)' }}>string</td>
                  <td style={{ color: 'var(--muted)' }}>Yes</td>
                  <td style={{ color: 'var(--muted)' }}>Card value in USD, as a decimal string (e.g. <Code>&quot;25.00&quot;</Code>).</td>
                </tr>
                <tr>
                  <td><Code>payment_asset</Code></td>
                  <td style={{ color: 'var(--muted)' }}>string</td>
                  <td style={{ color: 'var(--muted)' }}>No</td>
                  <td style={{ color: 'var(--muted)' }}><Code>&quot;usdc&quot;</Code> (default) or <Code>&quot;xlm&quot;</Code>. Determines the payment asset and returned quote.</td>
                </tr>
                <tr>
                  <td><Code>webhook_url</Code></td>
                  <td style={{ color: 'var(--muted)' }}>string</td>
                  <td style={{ color: 'var(--muted)' }}>No</td>
                  <td style={{ color: 'var(--muted)' }}>HTTPS URL to receive webhook POSTs on status changes.</td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  "budget": {
    ...
    "remaining_usdc": "85.00"
          ...
          <Para>
            ...
            quoted at the current market rate. The SDK calls <Code>pay_xlm()</Code> on the
            Soroban contract with this exact amount — no memo or address handling required.
          </Para>
          <CodeBlock label="Request">
{`POST /orders
X-Api-Key: cards402_your_key_here
...
{`{
  ...
  "budget": {
    ...
    "remaining_usdc": null
          ...
          <Para>
            The SDK handles both automatically. If using the MCP <Code>setup_wallet</Code> tool,
            the USDC trustline is added once the wallet has at least 2 XLM.
          </Para>

          <SubTitle>Payment window</SubTitle>
          <Para>
            Orders in <Code>pending_payment</Code> expire after <strong>2 hours</strong> if no
            on-chain payment is detected. Expired orders return <Code>phase: &quot;expired&quot;</Code>
            and no funds are taken. Create a new order to retry.
          </Para>

          <SubTitle>Overpayment</SubTitle>
          <Para>
            Send exactly the quoted amount. If you send more than the quoted amount, the excess
            is retained and will not be refunded. Underpayments are not matched — the order will
            expire after 2 hours and no card will be issued.
          </Para>
          ...
          >
            ...
            <Code>purchaseCardOWS()</Code> — it creates the order, signs and submits the
            Soroban transaction, and polls for the card in a single call. No contract interaction
            or order ID handling required.
          </div>
        </Section>
        ...
        <Section id="poll-order">
          ...
          <Para>
            Poll the status of an order. When <Code>status</Code> is <Code>&quot;delivered&quot;</Code>, the
            response includes the card object with PAN, CVV, and expiry.
          </Para>
...
{`{
  ...
  "updated_at": "2026-04-08T12:03:15.000Z"
        ...
        <Section id="order-statuses">
          ...
          <Para>
            Phase flow:{' '}
            <Code>awaiting_payment</Code> → <Code>processing</Code> → <Code>ready</Code>
          </Para>
          <Para>
          ...
          <Para style={{ color: 'var(--muted)', fontSize: '0.875rem' }}>
            ...
            state (e.g. <Code>ordering</Code>, <Code>stage1_done</Code>). Treat these as informational
            only — build your logic against <Code>phase</Code>, which is stable across backend changes.
          </Para>
        </Section>
        ...
        <Section id="webhooks">
          ...
          <Para>
            If you provide a <Code>webhook_url</Code> when creating an order, cards402 will POST
            to it when the order reaches <Code>delivered</Code> or <Code>failed</Code> status.
          </Para>
          <Para>
            Each webhook request includes two headers for verification:
          </Para>
          <ul style={{ color: 'var(--muted)', fontSize: '0.9rem', lineHeight: 1.7, paddingLeft: '1.5rem' }}>
            <li><Code>X-Cards402-Signature: sha256=&lt;hmac&gt;</Code> — HMAC-SHA256 over <Code>&lt;timestamp&gt;.&lt;body&gt;</Code></li>
            <li><Code>X-Cards402-Timestamp: &lt;unix-ms&gt;</Code> — send time in milliseconds</li>
          </ul>
          <Para>
            Always verify the signature and reject requests with a timestamp older than 5 minutes.
            Webhooks are retried automatically on failure: 30 seconds, 5 minutes, then 30 minutes
            (3 attempts total). Use polling as your primary status source — webhooks are a
            convenience notification, not a delivery guarantee.
          </Para>
...
{`{
  ...
  "message": "amount_usdc must be a positive number"
              ...
              <tbody>
                {errors.map(({ code, status, meaning }) => (
                  <tr key={code}>
                    ...
                    <td style={{ color: 'var(--muted)', fontFamily: 'var(--font-mono)', fontSize: '0.8125rem' }}>
                      {status}
                    </td>
                    <td style={{ color: 'var(--muted)', fontSize: '0.875rem' }}>{meaning}</td>
                  </tr>
        ...
        <Section id="rate-limits">
          ...
          <ul style={{ color: 'var(--muted)', fontSize: '0.9rem', lineHeight: 1.7, paddingLeft: '1.5rem' }}>
            <li><strong>Order creation</strong> — 60 per hour</li>
            <li><strong>Status polling</strong> — 600 per minute (10/s)</li>
          </ul>
          <Para>Exceeded limits return <Code>429 rate_limit_exceeded</Code>.</Para>
          <Para>
            However, each API key can have an optional <Code>spend_limit_usdc</Code> configured
            by the admin. Once the cumulative spend reaches this limit, the key returns{' '}
            <Code>403 spend_limit_exceeded</Code> until the limit is raised.
          </Para>
          <Para>
            If the fulfillment system has 3 consecutive failures, a circuit breaker freezes all
            new orders and returns <Code>503 service_temporarily_unavailable</Code> until an admin
          ...
          <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid var(--border)' }}>
            <p style={{ color: 'var(--muted)', fontSize: '0.875rem', margin: 0 }}>
              Questions?{' '}
              <a href="mailto:api@cards402.com" style={{ color: 'var(--green)', textDecoration: 'none' }}>
                api@cards402.com
              </a>
              {' · '}
              <Link href="/agents.txt" style={{ color: 'var(--muted)', textDecoration: 'none', fontFamily: 'var(--font-mono)' }}>
                /agents.txt
              </Link>
            </p>
          </div>
