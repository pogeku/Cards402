...
async function stage1_selectStore(page: Page): Promise<{ storeId: string; storeSlug: string }> {
  ...
  const postcodeInput = page
    .locator('input[placeholder="Enter your postcode"], input[placeholder*="postcode" i]')
    .first();
  ...
  const storeId =
    urlMatch?.[1] ??
    String((search.body as { localStore?: { id?: number } })?.localStore?.id ?? '');
...
async function stage3_checkout(page: Page, storeId: string, storeSlug: string) {
  ...
  if (details.status >= 400) {
    ...
    await page
      .locator('#FirstName, [name="FirstName"], [placeholder*="first name" i]')
      .first()
      .fill(ORDER.firstName)
      .catch(() => {});
    await page
      .locator('#LastName,  [name="LastName"],  [placeholder*="last name" i]')
      .first()
      .fill(ORDER.lastName)
      .catch(() => {});
    await page
      .locator('#Email,     [name="Email"],     [type="email"]')
      .first()
      .fill(ORDER.email)
      .catch(() => {});
    await page
      .locator('#PhoneNumber, [name="PhoneNumber"], [type="tel"]')
      .first()
      .fill(ORDER.phone)
      .catch(() => {});
    const addr = page
      .locator('#AddressLine1, [name="AddressLine1"], [placeholder*="address" i]')
      .first();
